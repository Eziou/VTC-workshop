...
y_train_tensor = torch.tensor(y_train, dtype=torch.float32).unsqueeze(1)  # 转换为 [batch_size, 1]
...
class GRUNet(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, dropout_rate):
        ...
        self.fc = nn.Linear(hidden_size, 1)  # 映射到一个标量（类别概率）

    def forward(self, x):
        h0 = torch.zeros(self.num_layers, x.size(0), self.hidden_size).to(x.device)
        out, _ = self.gru(x, h0)  # GRU 输出
        out = self.fc(out[:, -1, :])  # 取最后时间步的输出，映射到 [batch_size, 1]
        return out
...
model = GRUNet(X_train_tensor.size(1), hidden_size, num_layers, dropout_rate)
...
with torch.no_grad():
    for inputs, labels in test_loader:
        ...
        outputs = model(inputs)
        preds = torch.sigmoid(outputs).cpu().numpy()  # 应用 Sigmoid 函数，将输出映射到 [0, 1]
        all_preds.extend((preds > 0.5).astype(int))  # 预测类别
...
# 保存最终模型
model_path = '/Users/han/Charging station/model/215-GRU_sigmoid.pth'
