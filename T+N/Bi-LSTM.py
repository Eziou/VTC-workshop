...
class BiLSTMNet(nn.Module):
    ...
    def forward(self, x):
        h0 = torch.zeros(self.num_layers * 2, x.size(0), self.hidden_size).to(x.device)  # 双向LSTM的层数为 num_layers * 2
...
def random_search(X_train_tensor, y_train_tensor, param_distributions, num_samples):
    ...
    for params in param_sampler:
        ...
        # 初始化模型
        model = BiLSTMNet(X_train_tensor.size(1), hidden_size, num_layers, 2, dropout_rate)
        model.to(device)
