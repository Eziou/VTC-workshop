...
class GRUWithMLP(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, dropout_rate, mlp_hidden_size):
        ...
        self.mlp = nn.Sequential(
            ...
            nn.Linear(mlp_hidden_size, 2)  # 最终输出 2 个类别
        )
    ...
    def forward(self, x):
        ...
        out, _ = self.gru(x, h0)
        out = self.mlp(out[:, -1, :])  # 最后时间步输出传给 MLP
        return out
...
param_distributions = {
    ...
    'mlp_hidden_size': randint(64, 256)
}
...
def random_search(X_train_tensor, y_train_tensor, param_distributions, num_samples):
    ...
    for params in param_sampler:
        ...
        # 初始化模型
        model = GRUWithMLP(X_train_tensor.size(1), hidden_size, num_layers, dropout_rate, mlp_hidden_size)
        model.to(device)
...
# 保存最终模型
model_path = '/Users/han/Charging station/model/313——GRUWithMLP_model.pth'
