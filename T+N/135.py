...
y_train_tensor = torch.tensor(y_train, dtype=torch.float32).unsqueeze(1)  # Sigmoid expects shape (batch, 1)
...
# 定义 CNN 模型 + Sigmoid 层
class CNNNetWithSigmoid(nn.Module):
    def __init__(self, input_channels, input_length, dropout_rate):
        super(CNNNetWithSigmoid, self).__init__()
...
def random_search(X_train_tensor, y_train_tensor, param_distributions, num_samples):
    ...
    for params in param_sampler:
        ...
        # 初始化模型
        model = CNNNetWithSigmoid(1, X_train_tensor.size(2), dropout_rate)
        model.to(device)
...
# 保存最终模型
model_path = '/Users/han/Charging station/model/135——CNNWithSigmoid_model.pth'
