...
# 定义 CNN 模型 + Softmax 层
class CNNNetWithSoftmax(nn.Module):
    def __init__(self, input_channels, input_length, num_classes, dropout_rate):
        super(CNNNetWithSoftmax, self).__init__()
    ...
    def forward(self, x):
        ...
        x = torch.relu(x)
        x = self.pool(x)
        x = x.view(x.size(0), -1)  # Flatten for Softmax layer
        x = self.dropout(x)
        x = self.fc(x)
        x = self.softmax(x)  # Apply Softmax to output probabilities
        return x
...
def random_search(X_train_tensor, y_train_tensor, param_distributions, num_samples):
    ...
    for params in param_sampler:
        ...
        # 初始化模型
        model = CNNNetWithSoftmax(1, X_train_tensor.size(2), 2, dropout_rate)
        model.to(device)
...
# 保存最终模型
model_path = '/Users/han/Charging station/model/134——CNNWithSoftmax_model.pth'
