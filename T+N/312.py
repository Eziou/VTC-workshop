...
class GRUWithFC(nn.Module):
    ...
    def forward(self, x):
        ...
        out, _ = self.gru(x, h0)
        out = self.fc(out[:, -1, :])  # 取最后时间步的输出并传入全连接层
        return out
...
def random_search(X_train_tensor, y_train_tensor, param_distributions, num_samples):
    ...
    for params in param_sampler:
        ...
        # 初始化模型
        model = GRUWithFC(X_train_tensor.size(1), hidden_size, num_layers, 2, dropout_rate)
        model.to(device)
...
# 保存最终模型
model_path = '/Users/han/Charging station/model/312—GRUWithFC_model.pth'
