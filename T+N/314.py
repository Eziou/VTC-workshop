...
class GRUWithSoftmax(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, dropout_rate):
        ...
        self.fc = nn.Linear(hidden_size, 2)  # 输出层
    ...
    def forward(self, x):
        ...
        out = self.softmax(out)  # 应用 Softmax
        return out
...
def random_search(X_train_tensor, y_train_tensor, param_distributions, num_samples):
    ...
    for params in param_sampler:
        ...
        # 初始化模型
        model = GRUWithSoftmax(X_train_tensor.size(1), hidden_size, num_layers, dropout_rate)
        model.to(device)
...
# 保存最终模型
model_path = '/Users/han/Charging station/model/314——GRUWithSoftmax_model.pth'
