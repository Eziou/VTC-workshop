...
y_train_tensor = torch.tensor(y_train, dtype=torch.long)  # 对于 CrossEntropyLoss，需要使用 long 类型标签
...
class GRUNet(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_classes, dropout_rate):
        ...
        self.fc = nn.Linear(hidden_size, num_classes)  # 全连接层映射到类别数
        self.softmax = nn.Softmax(dim=1)  # Softmax 层

    def forward(self, x):
        h0 = torch.zeros(self.num_layers, x.size(0), self.hidden_size).to(x.device)
        out, _ = self.gru(x, h0)
        out = self.fc(out[:, -1, :])  # 最后时间步输出 [batch_size, hidden_size] -> [batch_size, num_classes]
        out = self.softmax(out)  # 输出类别概率
        return out
...
model = GRUNet(X_train_tensor.size(1), hidden_size, num_layers, num_classes, dropout_rate)
...
with torch.no_grad():
    for inputs, labels in test_loader:
        ...
        outputs = model(inputs)
        preds = torch.argmax(outputs, dim=1).cpu().numpy()  # 取概率最大的类别作为预测
...
# 保存最终模型
model_path = '/Users/han/Charging station/model/214——GRU_with_softmax.pth'
