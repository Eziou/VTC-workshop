...
# 定义 LSTM 模型 + 全连接层
class LSTMNetWithFC(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_classes, dropout_rate):
        super(LSTMNetWithFC, self).__init__()
    ...
    def forward(self, x):
        ...
        out = self.fc(out[:, -1, :])  # 取最后时间步的输出，并通过全连接层
        return out
...
def random_search(X_train_tensor, y_train_tensor, param_distributions, num_samples):
    ...
    for params in param_sampler:
        ...
        # 初始化模型
        model = LSTMNetWithFC(X_train_tensor.size(1), hidden_size, num_layers, 2, dropout_rate)
        model.to(device)
...
# 保存最终模型
model_path = '/Users/han/Charging station/model/122——LSTMWithFC_model.pth'
