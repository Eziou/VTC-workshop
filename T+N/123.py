...
# 定义 LSTM 模型 + MLP 分类器
class LSTMNetWithMLP(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, mlp_hidden_size, num_classes, dropout_rate):
        super(LSTMNetWithMLP, self).__init__()
    ...
    def forward(self, x):
        ...
        out = self.mlp(out[:, -1, :])  # 取最后时间步的输出，并通过 MLP 分类器
        return out
...
param_distributions = {
    ...
    'mlp_hidden_size': randint(64, 128),
...
def random_search(X_train_tensor, y_train_tensor, param_distributions, num_samples):
    ...
    for params in param_sampler:
        ...
        # 初始化模型
        model = LSTMNetWithMLP(X_train_tensor.size(1), hidden_size, num_layers, mlp_hidden_size, 2, dropout_rate)
        model.to(device)
...
# 保存最终模型
model_path = '/Users/han/Charging station/model/123——LSTMWithMLP_model.pth'
