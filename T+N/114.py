...
# 定义 GRU 模型 + Softmax 层
class GRUNetWithSoftmax(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_classes, dropout_rate):
        super(GRUNetWithSoftmax, self).__init__()
...
def random_search(X_train_tensor, y_train_tensor, param_distributions, num_samples):
    ...
    for params in param_sampler:
        ...
        # 初始化模型
        model = GRUNetWithSoftmax(X_train_tensor.size(1), hidden_size, num_layers, 2, dropout_rate)
        model.to(device)
...
# 保存最终模型
model_path = '/model/114——GRUWithSoftmax_model.pth'
