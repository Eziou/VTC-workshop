...
# 定义 GRU 模型 + Sigmoid 层
class GRUNetWithSigmoid(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, dropout_rate):
        super(GRUNetWithSigmoid, self).__init__()
...
def random_search(X_train_tensor, y_train_tensor, param_distributions, num_samples):
    ...
    for params in param_sampler:
        ...
        # 初始化模型
        model = GRUNetWithSigmoid(X_train_tensor.size(1), hidden_size, num_layers, dropout_rate)
        model.to(device)
...
# 保存最终模型
model_path = '/Users/han/Charging station/model/115——GRUWithSigmoid_model.pth'
