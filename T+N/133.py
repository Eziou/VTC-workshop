...
X_test_tensor = torch.tensor(X_test, dtype=torch.float32).unsqueeze(1)  # 增加通道维度
...
# 定义 CNN 模型 + MLP 分类器
class CNNNetWithMLP(nn.Module):
    def __init__(self, input_channels, input_length, mlp_hidden_size, num_classes, dropout_rate):
        super(CNNNetWithMLP, self).__init__()
        ...
        # 动态计算 MLP 输入维度
        self.mlp_input_dim = 128 * (input_length // 4)
        self.mlp = nn.Sequential(
            nn.Linear(self.mlp_input_dim, mlp_hidden_size),
            nn.ReLU(),
    ...
    def forward(self, x):
        ...
        x = torch.relu(x)
        x = self.pool(x)
        x = x.view(x.size(0), -1)  # Flatten for MLP layer
        x = self.dropout(x)
...
def random_search(X_train_tensor, y_train_tensor, param_distributions, num_samples):
    ...
    for params in param_sampler:
        ...
        # 初始化模型
        model = CNNNetWithMLP(1, X_train_tensor.size(2), mlp_hidden_size, 2, dropout_rate)
        model.to(device)
...
# 保存最终模型
model_path = '/Users/han/Charging station/model/133——CNNWithMLP_model.pth'
