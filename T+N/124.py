...
# 定义 LSTM 模型 + Softmax 层
class LSTMNetWithSoftmax(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_classes, dropout_rate):
        super(LSTMNetWithSoftmax, self).__init__()
    ...
    def forward(self, x):
        ...
        out = self.softmax(out)  # 使用 Softmax 函数计算类别概率
        return out
...
def random_search(X_train_tensor, y_train_tensor, param_distributions, num_samples):
    ...
    for params in param_sampler:
        ...
        # 初始化模型
        model = LSTMNetWithSoftmax(X_train_tensor.size(1), hidden_size, num_layers, 2, dropout_rate)
        model.to(device)
...
# 保存最终模型
model_path = '/Users/han/Charging station/model/model/124——LSTMWithSoftmax_model.pth'
