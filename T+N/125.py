...
y_train_tensor = torch.tensor(y_train, dtype=torch.float32).unsqueeze(1)  # Sigmoid requires shape (batch, 1)
...
# 定义 LSTM 模型 + Sigmoid 层
class LSTMNetWithSigmoid(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, dropout_rate):
        super(LSTMNetWithSigmoid, self).__init__()
    ...
    def forward(self, x):
        ...
        out = self.fc(out[:, -1, :])  # 取最后时间步的输出并通过全连接层
        out = self.sigmoid(out)  # 使用 Sigmoid 函数计算概率
        return out
...
def random_search(X_train_tensor, y_train_tensor, param_distributions, num_samples):
    ...
    for params in param_sampler:
        ...
        # 初始化模型
        model = LSTMNetWithSigmoid(X_train_tensor.size(1), hidden_size, num_layers, dropout_rate)
        model.to(device)
        ...
        with torch.no_grad():
            for inputs, labels in test_loader:
                inputs, labels = inputs.unsqueeze(1).to(device), labels.to(device)
                outputs = model(inputs)
                all_preds.extend((outputs.cpu().numpy() > 0.5).astype(int))  # 二分类
...
# 保存最终模型
model_path = '/Users/han/Charging station/model/125——LSTMWithSigmoid_model.pth'
