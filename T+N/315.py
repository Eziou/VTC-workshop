...
y_train_tensor = torch.tensor(y_train, dtype=torch.float32).unsqueeze(1)  # 增加维度用于二分类
X_test_tensor = torch.tensor(X_test, dtype=torch.float32)
y_test_tensor = torch.tensor(y_test, dtype=torch.float32).unsqueeze(1)  # 增加维度用于二分类
...
# 定义特征加权层
class FeatureWeightingLayer(nn.Module):
    def __init__(self, num_features):
        super(FeatureWeightingLayer, self).__init__()
        self.weights = nn.Parameter(torch.ones(num_features))  # 初始化权重为 1

    def forward(self, x):
        weighted_x = x * self.weights  # 对输入特征加权
        return weighted_x

# 定义 GRU + Sigmoid 模型
class GRUWithSigmoid(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, dropout_rate):
        super(GRUWithSigmoid, self).__init__()
        self.feature_weighting = FeatureWeightingLayer(input_size)  # 加权层
        self.hidden_size = hidden_size
        self.num_layers = num_layers
        self.gru = nn.GRU(input_size, hidden_size, num_layers, batch_first=True, dropout=dropout_rate)
        self.fc = nn.Linear(hidden_size, 1)  # 单一输出用于二分类
        self.sigmoid = nn.Sigmoid()  # Sigmoid 层

    def forward(self, x):
        x = self.feature_weighting(x)  # 应用特征加权
        h0 = torch.zeros(self.num_layers, x.size(0), self.hidden_size).to(x.device)
        out, _ = self.gru(x, h0)
        out = self.fc(out[:, -1, :])  # 取最后时间步的输出
        out = self.sigmoid(out)  # 应用 Sigmoid
        return out
...
def random_search(X_train_tensor, y_train_tensor, param_distributions, num_samples):
    ...
    for params in param_sampler:
        ...
        # 初始化模型
        model = GRUWithSigmoid(X_train_tensor.size(1), hidden_size, num_layers, dropout_rate)
        model.to(device)
...
# 保存最终模型
model_path = '/Users/han/Charging station/model/315——GRUWithSigmoid_model.pth'
