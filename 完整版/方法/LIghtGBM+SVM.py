...
class TwoLayerDTSVM:
    def __init__(self, epochs=10):
        ...
        self.best_params = self.svm_params.copy()
    ...
    def train_dt(self, X, y):
        ...
        model = DecisionTreeClassifier(**self.dt_params)
        model.fit(X, y)
        end_time = time.time()
        print(f"Layer 1 (Decision Tree) training time: {end_time - start_time:.4f} seconds")
        return model

    def train_svm(self, X, y, layer_name, sample_weight=None):
        params = self.svm_params[layer_name]
    ...
    def fit(self, X, y, sample_weight=None):
        # 训练第一层决策树
        print('Training Layer 1 (Decision Tree)...')
        self.dt_layer1 = self.train_dt(X, y)

        # 第一层预测训练集标签
        labels_layer1 = self.dt_layer1.predict(X)
        ...
        if len(X_s0) > 0 and len(np.unique(y_s0)) > 1:
            self.svm_layer2_s0 = self.train_svm(
                X_s0, y_s0, 'Layer 2 (S_0)',
        ...
        if len(X_s1) > 0 and len(np.unique(y_s1)) > 1:
            self.svm_layer2_s1 = self.train_svm(
                X_s1, y_s1, 'Layer 2 (S_1)',
    ...
    def predict(self, X):
        ...
        if self.svm_layer2_s0 is not None:
            y_pred[mask_s0] = self.svm_layer2_s0.predict(X[mask_s0])
        if self.svm_layer2_s1 is not None:
...
print(f"Best SVM parameters: {model.best_params}")

# 保存结果到 CSV
results = {
    'Metric': ['Accuracy', 'Recall', 'Precision', 'F1 Score', 'Total Training Time (s)',
               'Optimization Time (s)', 'Prediction Time (s)'],
    'Value': [accuracy, recall, precision, f1, total_train_time, optimization_time, predict_time]
}
params = {
    'Layer': ['Layer 2 (S_0)', 'Layer 2 (S_1)'],
    'Best Parameters': [str(model.best_params.get('Layer 2 (S_0)', {})),
...
output_file = os.path.join(os.getcwd(), 'two_layer_dt_svm_results.csv')
